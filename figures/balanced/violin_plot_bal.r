...
data <- read.csv("~/Documents/Personomics_Research_Group/WAPIAW_2/figure_plotting/features_bal/plotting.csv")

# default output file (png and optionally pdf) name (without file extension)
fname <- "~/Documents/Personomics_Research_Group/WAPIAW_2/figure_plotting/all_plots/features_bal_plot"
print(fname)
...
data$method <- factor(
	...
	"Concatenated & Amplitude & PhenoAvg",
        "Concatenated & Amplitude & PhenoAvg & Balance"),
    labels = c("Rest (Dadi et al. 2021)", "Rest (replication)",
 	"Rest & Balance",
	"Concat & Amp & PhenoAvg",
        "All & Balance"),
)
...
fig <- ggplot(
    ...
    ggtitle("Approximation Quality: Brain Data on Balanced Targets") +
